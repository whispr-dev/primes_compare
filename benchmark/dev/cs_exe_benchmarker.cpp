...
#include <algorithm>
#include <sstream>
#include <cmath>  // Added this for std::sqrt

// Improved process execution with timeout
static std::string exec(const char* cmd, int timeout_ms = 5000) {
...
private:
    struct Program {
        ...
        std::vector<long long> run_times;  // Store all times for statistical analysis
    };
...
public:
    ExeBenchmarker(int runs = 10, int repeats = 10, bool verbose_output = false)
        : num_runs(runs), repeats_per_run(repeats), verbose(verbose_output) {}
            ...
            for (auto& program : programs) {
                ...
                try {
                    ...
                    for (int r = 0; r < repeats_per_run; ++r) {
                        ...
                        exec(cmd.c_str(), 5000);  // 5 second timeout
...
int main() {
    try {
        ExeBenchmarker benchmark(10, 10, true);  // 10 runs, 10 repeats, verbose output

        // Add your executables to benchmark
        benchmark.addProgram("p-primes.exe", "C:/users/phine/desktop/primes_ben/dist/p-primes.exe");
        benchmark.addProgram("p-primes-fast.exe", "C:/users/phine/desktop/primes_ben/dist/p-primes-fast.exe");
        benchmark.addProgram("p-primes-faster.exe", "C:/users/phine/desktop/primes_ben/dist/p-primes-faster.exe");
        benchmark.addProgram("p-primes-fastest.exe", "C:/users/phine/desktop/primes_ben/dist/p-primes-fastest.exe");
        benchmark.addProgram("c-primes.exe", "C:/users/phine/desktop/primes_ben/dist/c-primes.exe");
        benchmark.addProgram("c-primes-fast.exe", "C:/users/phine/desktop/primes_ben/dist/c-primes-fast.exe");
        benchmark.addProgram("r-primes.exe", "C:/users/phine/desktop/primes_ben/dist/r-primes.exe");
        benchmark.addProgram("r-primes-fast.exe", "C:/users/phine/desktop/primes_ben/dist/r-primes-fast.exe");
